...
firma Parrot \cite{electronic:parrot, electronic:wiki_parrot}.%\cite{electronic:parrot} \cite{electronic:wiki_parrot}.
...
\subsection{Kommunikation mellem \drone{}n og en klient-enhed}
...
\section{Vores systems kommunikation med AR.Dronen} %Brugerens interaktionsmuligheder med systemet}
...
\subsection{Opbygningen af det grundlæggende controllermodul}
...
\section{Introduktion til modtagelse af video\-, navigations\- og wifi\-data fra AR.\ $\!$Dronen}
...
\subsection{Eksempel på anvendelse af videoreceiveren}
...
\todo{fedt hvis vi kan nå at lave det...}



\section{Korridors forsvindingspunkt}
...
Kanter i et billede defineres som abrupt skift i farveværdi eller lysintensitet lokalt mellem
naboer af pixels\footnote{Man kan også lede efter kanter globalt (se f.eks.\newline
  \uri{http://en.wikipedia.org/wiki/Step\_detection\#Global}), men det
  er en anden tilgang. I dette afsnit beskrives en metode der søger lokalt.}. Kanter er ofte en
...
Cannyfilteret\cite{Canny1986}, som er en af de metoder vi bruger, tager et billede og returnere et binært billede (sort/hvid). Hvide billedpunkter indikere at der er en kant i det oprindellige billede, mens sort betyder at der ikke er ændringer. Cannyfilteret bruger gradienten som skrives
\begin{equation}
  \label{eq:edge_slope}
  %\bigtriangledown
  \bm{J} = \bigtriangledown I (\bm{x} ) = \left(\frac{ \partial I }{ \partial x }, \frac{
    \partial I }{ \partial y }\right)(\bm{x})
\end{equation}
og er ændringen i intensiteten i $x$ og i $y$ retningen. Udfra denne fåes magnituden og orienteringen af gradienten som
\begin{equation}
  \label{eq:gradient}
  \begin{split}
  G &= \sqrt{ G_x^2 + G_y^2 }\\
  \theta &= \arctan \left( \frac{G_y}{G_x} \right)\\
  \text{ hvor } G_x & = \frac{ \partial I }{ \partial x } \text{ og } G_y \frac{\partial I }{ \partial y }
  \end{split}
\end{equation}
Cannyfilterets arbejde besværliggøres af støj i rå billeder. Støjen introducerer fejl i lokationsestimatet for kanterne. En reducering af støjen kan fortages forinden eller imens beregning af hældning
foregår. Specielt i tilfældet med AR.Dronens ringe kamera vil det give et bedre slutresultat at bruge et båndpasfilter til at lave en ``udglatning'' af billedet. Udglatningsoperationen udføres med et gaussisk filter\footnote{Fra billedbehandlingsprogrammer som Photoshop og GIMP hedder det ``Gaussian blur'' filter.} (en klokkegraf).
%convolution oversat til dansk; en foldning. = sliding window (gauss bruges sådan)

Gradienten af gauss-udglatningen convoleres på billedets pixelintensiteter og skrives som
\begin{equation}
  \label{eq:gauss_edge_slope}
  \bm{J}_{\sigma} = \bigtriangledown [ G_{\sigma} (\bm{x} ) \times I
    (\bm{x} ) ] = 
     [ \bigtriangledown G_{\sigma} ] (\bm{x} ) \times I (\bm{x} )
\end{equation}
...
Figur~\ref{fig:derivatives} illustrerer et eksempel hvor lysintensitet ($f(x)$) per pixelposition $x$ går fra lav værdi til høj værdi. Kanten skal findes i skiftet hvor $f(x)$ går mellem at være tiltagende til at være aftagende; der er den største ændring (mest abrupt). Er gradienten, altså ændringen i punktet ($f'(x)$) over en hvis værdi (threshold) antages der at være en kant i området. I gradienten kan man begynde at lede efter lokale optimum; hvor der er lokalt optimum er der som sagt en kant. Det originale cannyfilter kan udbygges med brugen af den anden afledtes skærring med 0, som skrives
\begin{equation}
  \label{eq:gradient2gauss}
  \bigtriangledown ^2 G_{\sigma} (\bm{x} ) = \frac{1}{\sigma ^3} \left( 2 - \frac{x^2 + y^2}{2 \sigma ^2} \right) \exp 
  \left( - \frac{x^2 + y^2}{2 \sigma ^2} \right) 
\end{equation}

% Lindeberg98 edges found by second and third derivatives
% second derivative = laplacian
%Se $f(x)$ som lysintensitet per pixel (hvor $x$ er pixelposition i vandret plan), og $f'(x)$ som gradient, mens den anden afledte; $f''(x)$ er ændringen i ændringen
