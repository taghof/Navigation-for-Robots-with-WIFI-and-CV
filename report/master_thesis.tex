...
\subsection{Motivation} % Thomas & Morten, Hvorfor
...
\subsection{Teseformulering/mål} % Thomas & Morten, Hvad
\label{sec:tese}
...
\subsection{Metode} % Thomas, Hvordan, skal den være en SUBsection eller section?
\label{sec:metode}
...
%%%% Introduction - Metode
\subsubsection{Domæne/navigationsmiljø} % Thomas,
Nogle bygninger på Katrinebjerg, indendørs...

\subsubsection{/Miljø} % Thomas,
\paragraph{Indendørs}
...
%%%% Introduction - Metode
\subsubsection{Platform samt hjælpeværktøjer} % Thomas & Morten
...
\subsubsection{Eksperimenter} % Thomas,
...
\subsection{Vurdering af AR.Dronen som robotplatform} % Thomas,
...
Eksperimenter med hastighedsudledning er beskrevet i afsnit~\ref{sec:distance-mesu}. Det viser sig at under de rette
forhold er hastighedsudledningen udmærket brugbar. Er det derimod ikke muligt for bundkameraet at opfange nok tydelige kendetegn i gulvet, vil hastighedudledningen være fejlbehæftet.
\paragraph{Ultralydssensor}
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.80\textwidth]{grafik/sonardzone2.png}
  \caption{Udbredelsesområde for ultralydssensorsignal, man ser at afstanden ikke nødvendigvis er vinkelret på sensoren idet der kan forekomme ekko fra et større område under sensoren (figuren er fra \uri{http://www.cs.brown.edu/people/tld/courses/cs148/02/sonar.html}).\todo{i referencer?}}
  \label{fig:sonicbeam}
...
\end{figure}
Højdemåleren består som sagt en ultralyds sender og modtager, teknologien gør at %specielt i små rum/kontorlokaler, små kasser / borde + turbolens
 højden ikke er udfra et specifikt punkt, men derimod returnerer den mindste højde indenfor et område som det på figur \ref{fig:sonicbeam2}. Dvs. højden er udledt af den tid der går fra en lydpuls afsendes til der modtages et ekko. Ekkoet behøver ikke komme fra en stor overflade som en væg eller et gulv, men kan være en kasse inde i sensorområdet som figur~\ref{fig:sonicbeam2}.
Fordi teknologien netop er lyd vil der også være indeboende problemer med ekko på skrånende og bløde overflader. 
%
...
\subsection{Fjernstyring af AR.Dronen med Python}
%        Introduktion til styring af AR.\ $\!$Drone med Python}

\subsubsection{AT-kommando Python interface}
...
gennem interfacet, behøves vi ikke heller ikke at tænke på detaljer som
...
\pagebreak
%        Vores systems kommunikation med AR.Dronen
\subsection{Styring af AR.Dronen gennem den ny platform} %Brugerens interaktionsmuligheder med systemet}
Som nævnt i sektion~\todo{\\ref{SEKTIONNOGET}}
 kanaliserer vores system al
kontrolkommunikation med dronen gennem kontrolinterfacet, i praksis er
det dog ikke anvendeligt at brugeren skal afvikle scripts, der anvender
kontrolinterfacet, for hver ønsket bevægelse(som det ses i
figur~\ref{fig:pythonimpl1}), i stedet har vi implementeret
moduler (controllere) til at tage mod input fra brugeren. Vi 
har tre inputmuligheder: En keyboardcontroller der lytter efter input
fra tastaturet, en joystickcontroller der er tilknyttet en fysisk
Xbox360-controller (se figur~\ref{fig:xboxlayout}) samt en autocontroller der kan initiere og afslutte
givne opgaver. 

Vi har mulighed for at starte systemet op med alle
...
skulle ske noget uforudset under udførslen af en opgave, vil der ikke
være nogen mulighed for at bringe AR.Dronen manuelt ned. 

\subsubsection{Opbygningen af det grundlæggende controllermodul}%styringsmodul?
Alle tre controllermoduler nedarver fra en grundlæggende
controllerklasse (Controller), denne er implementeret som en tråd der med et givent
tidsinterval kalder en kontrolmetode(\code{process\_events()}). Denne simple
...
TASK SECTION) på autocontrolleren, således kan piloten manuelt bringe
AR.Dronen i en ønsket position for herefter at starte den automatiske
...
\subsection{Autocontroller}
Autocontrolleren skiller sig ud fra de andre controllere, den nedarver
stadig samme grundlæggende controllerklasse, men i modsætning til
joystick- og keyboardcontrollerne arbejder autocontrolleren ikke
direkte med kontrolinterfacet. Autocontrollerens opgave er at starte
forudspecificerede opgaver(tasks) og stoppe dem igen hvis det er nødvendigt 
...
tilkoblet (hvilket dog vil resultere i begrænset
funktionalitet). %Wut? begrænset?
 Sektion~\ref{subsection:receiver} giver et eksempel
...
\section{Eksperimenter} % Thomas & Morten
...
\subsection{Lokalisering via blobs} % Thomas & Morten
...
\subsection{Korridors forsvindingspunkt} % Thomas
%Hvad vil vi vise, afvise teste?
%Hvad kan/skal resultatet bruges til?
%Hvordan løste vi det, undersøgte det, hvordan udførte vi eksperimentet?
%Hvad var resultatet?
...
\subsubsection{At finde kanter i et billede}
...
\subsection{Distance-measurererererererere} % Thomas
...
\subsection{Inhibition/subsumtion} % Morten
Hvad vil vi vise, afvise teste?
Hvad kan/skal resultatet bruges til?
Hvordan løste vi det, undersøgte det, hvordan udførte vi eksperimentet?
Hvad var resultatet?
...
\subsection{Erkendelse af miljø/mode...}
Hvad vil vi vise, afvise teste?
Hvad kan/skal resultatet bruges til?
Hvordan løste vi det, undersøgte det, hvordan udførte vi eksperimentet?
Hvad var resultatet?
...
\section{Medfølgende CD / internet}
