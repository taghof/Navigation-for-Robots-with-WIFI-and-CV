...
firma Parrot\cite{electronic:parrot}\cite{electronic:wiki_parrot}. En quadrocopter benytter fire faste rotorer til at
generere opdrift, samt til at styre sin bevægelse i rummet.%\\

AR.Drone blev introduceret i 2010 som ``the Flying Videogame'' og promoveres stadig af producenten som et stykke legetøj. Det har siden fundet stor anvendelse, ikke blot som et avanceret stykke legetøj. Forskere, studerende og hobbyister har taget den fjernstyrede quadrocopter til sig som en færdig platform til at udvikle videre ovenpå\cite{electronic:ludep} og som platform til selv at ombygge.%\cite evt. flere som har brugt AR.Dronen i deres forsøg forskning, bl.a. LUDEP.

Parrots quadrocopter kommer med et sæt sensorer bestående af to kameraer; et vertikalt 
...
\subsection{Hardware}
\label{ssec:hardware}
Som de fleste robotplatforme består AR.Drone af et antal sensorer, et antal
...
3-akses accelerometer i en samlet enhed til at måle inerti (IMU), denne anvendes til
...
\subsection{Fysik}
\label{ssec:fysik}
AR.Dronen fåes som sagt med 2 skrog eller skjold (figur~\ref{fig:indendoersskjold}), og vejer 420 gram med det indendørs skrog monteret. Et pund er en relativ lille vægt, andre\cite{quteprints33767} quadrokoptere har f.eks. 4-5 kg som de skal løfte rundt på.

De 4 propeller sidder i hver sit hjørne af et kulfiberkryds på AR.Dronen. Når en propel på AR.Dronen rotere om sin akse flytter den luften i et areal rundt om propellen. Lufttrykket på undersiden af propellen bliver herved større end lufttrykket på oversiden af propellen. Hvis AR.Dronen ligger vandret vil forskellen i lufttryk giver en Bernoullieffekt\cite{electronic:wiki_bernoulli} som påvirker AR.Dronen med en kraft modsat tyngdekraften (thrust)\cite{electronic:wiki_quadrotor}. Udover thrust påvirker den roterende bevægelse også propellens base med et moment (torque) modsat propellens retning. Eksempelvis vil en helikopter uden haleror dreje ukontrolerbart rundt om sig selv, men fordi AR.Drone har et par rotore som drejer een vej (med uret), mens et andet par drejer modsat (imod uret)\cite{techreport:ardroneDevGuide}, bliver summen af momentpåvirkning nul, hvis de 4 rotere drejer med samme hastighed.
...
IP\cite{rfc791}-adresser og lave et netværk mellem en klient og AR.Dronen selv, samt en telnet\cite{frc854}-server der giver adgang til AR.Dronens terminal %telnet cite?
og en texteditor %(Vi)
...
AR.Dronen under flyvning. -- Program.elf indeholder blandt andet
\begin{itemize}
\item en PID controller til at stabilisere AR.Dronen når der svæves, 
\item en algoritme til at udlede AR.Dronens hastighed ved hjælp af bundkameraet (bekræftes også i en artikel af Parrot selv\cite{velocity_estimation_inside_the_drone}.)%cite hvor
...
reality spil. 
\end{itemize}
Program.elf modtager kommandoer fra brugeren gennem en User Datagram Protocol\cite{rfc768} (UDP) 
port. Kommunikationsinterfacet mellem bruger og quadrokopteren beskrives i afsnit~\ref{ssec:kommunikation}.

\subsection{Kommunikation med AR.Dronen}
...
\begin{itemize}
  \item som klient (PC, smartphone eller lignende) via Telnet eller FTP\cite{frc959} til AR.Dronen, hvor AR.Dronen fungere som server,
  \item ved at eksekvere en applikation på en klient (PC, smartphone eller lignende) der sender og modtager UDP-pakker og AT-kommandoer\footnote{AT(tention)-kommandoer gennemgåes nærmere i sektion \ref{sssec:atinterface}} til det kørende firmware \url{program.elf} på AR.Dronen,
  \item eller ved at eksekvere et program på selve AR.Dronen der kommunikerer over 3 UDP-porte og køre parallelt med firmware'en \url{program.elf} på AR.Dronen.
\end{itemize}
...
PUT og GET kommandoer. Hvis klienten ikke angives en sti lægges 
overførte filer som standard i \url{~/data/video} mappen.

\subsubsection{UDP baseret kommunikation}
\label{sssec:udp}
I modsætning til FTP- og Telnet-klienterne, som er TCP baserede, anvendes
UDP-protokollen til den kommunikation der sker mellem \url{program.elf} og
brugerens system (iPhone eller PC).

Firmware'en \url{program.elf} på AR.Dronen sender og modtager pakker på 3 UDP-porte:
\begin{itemize}
  \item På port 5556 lytter quadrokopteren efter brugerkommandoer, e.g. takeoff, flyv frem, drej, land osv.
  \item fra port 5555 sendes
  \item fra port 5554 
\end{itemize}
...
En bruger applikation kommunikerer med AR.Dronens software ved hjælp af
såkaldte AT-kommandoer, En AT-kommando er basalt set blot en
tekststreng encodet som 8 bits karakterer, kommandoer sendes til AR.Dronens
