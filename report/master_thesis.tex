...
af subjektiv sensning fra ombord på quadrokopteren. Eksempeler på
...
eksperimenterne af Chen et al omkring naviagtion af AR.Dronen i
forskellige  indendørs miljøer,~\cite{single_img_pespective_cues}.
Ideen om at sansning og orientering skal komme fra quadrokopteren selv
ses stadig så sent som 2011 hos chen et al. i deres forsøg med
...
ønskede at lære mere om og få mere erfaring med beregning indvirkning
i den fysiske verden. Vi havde en forventning og håb om om at den
fysiske verden ville stritte imod vores modelrepræsentationer af
verdenen. 
...
Igennem vores forundersøgelser af AR.Dronen er vi blevet overasket
...
brugere, som ikke nødvendigvis er programmøre. Et perspektiv kunne
...
forstå AR.Dronen og dens muligheder. Det handler om at lave en lille
smule grundforskning i et proof-of-concept. Vi vil gerne forstå
...
To højt prioriteret krav til specialet er at 
\begin{itemize}
\item 1 der skal konstruere en softwareplatform til at lette de videre
  eksperiementer i specialet.
%(, 3) mediere kontrollen med AR.Dronen, 
\item 2 det skal undersøges hvorvidt en quadrokopter der er designet
  til kontrol via fjernstyring kan gøres til semi-autonomt at navigere
  i et indendørs miljø.
\end{itemize}
Derudover skal der undersøges om
\begin{itemize}
\item 3 platformen kan konstrueres til at tilgængeliggøre et let
  anvendelig, medierende interface til AR.Dronen 
\item 4 mulighederne for at bruge AR.Dronens ombordværende
  sensorsystem (udvidede sensorsystem) til at udlede højere ordens
  viden om navigationsmiljø er anvendelige, og om den ny viden kan
  bruges som støtte i den autonome styring af quadrokopteren. Herunder
  skal det konkret undersøges mulighederne for at bruge:
  \begin{itemize}
  \item positionering via wifi
  \item positionering via visuelle markøre
  \item detektering af navigationsmiljøtype
  \item odometrisk afstandsmåling
  \item optisk triangulering af afstande
  \end{itemize}
\item 5 AR.Dronen faktisk er anvendelighed som robotplatform.
\end{itemize}

\todo{skal der evt tilføjes noget med understøttelse af inhibition?
  andre uddybende krav til selve platformen?}
...
at finde læsninger på, 
%
men i spørgsmålet om at finde løsninger til naviagtionsproblemet alene
findes allerede mange svar der ligner. Der er skrevet meget litteratur
der beskriver løsninger som ligner navigation i Minerva. 
%
...
kan besøge museet fra internettet. Spørgsmålet er hvile implikationer
...
stort sikkerhedsaspekt i det at lade en quadrokopter næsten
selvstyrende flyve rundt i blandt mennesker. En anden mulighed er at
...
indenfor området computer
vision,~\cite{opencv}\cite{opencv:wiki}. OpenCV blev oprindeligt
startet af Intel(med bidrag fra optimeringseksperter i Intel Russia og
...
\subsection{Eksperimenter} For at teste AR.Dronen og vores
...
\paragraph{Lokalisering via visuelle markører} simpel blob, og
udbygget nested blob Vi har designet et antal forskellige visuelle
markører. Markørerne er blevet testet for hvor godt de kunne genkendes
af AR.Dronen, både i statiske opstillinger og i forbindelse med
bevægelse. Disse eksperimenter har også fungeret som test af vores
tasksystem og de udviklede tasks. Eksperimenternes opstilling og
resultater kan ses i sektion~\ref{ssec:visuellokalisering}.
...
På den medfølgende CD findes kildekoden til den udviklede
klientplatform inklusiv de udviklede tasks.
...
har det været nødvendigt, at kunne følge udviklingen i wifisignalstyrker
...
flest skæringer er over en vis størrelse (en grænse på 100 har vist
godt resultat).
%
...
tilhørende forsvindingspunkt. Observationer har indikeret at detektoren
kan gøres mere robust ved yderligere at analysere på om fordelingen af
linieskæringspunkter er jævnt fordelt over det meste af billedet (i
diagonalt mod forsvindingspunkt) og
eksempelvis ikke kun er koncentreret omkring en enkelt linie eller
enkelt område.
...
Dette eksperiment undersøger virkningsgraden af en odometrisk
...
tacho-counter i motoren på nogle kørende robotter.  
...
FollowTourTask (sektion~\ref{sssec:followtourtask}). 
...
Resultaterne af flyvningerne kan ses på figur~\ref{fig:xdistr} og
...
\chapter{Konklusion}
\paragraph{Den udviklede klientplatform} har været det primære arbejdsredskab
under ekperimenterne med AR.Dronen. På trods af kontinuerlig udvikling
under hele projektet, har klientplatformen understøttet
eksperimenterne ved at give let adgang til AR.Dronens sensorstrømme og
ved at tilbyde task-abstraktionen ovenpå kontrolinterfacet. Det
udviklede tasksystem har været så fleksibelt, at det både har kunnet
anvendes til simple, tidsbegrænsede, flyvninger i en specifik retning
og til sammensatte navigationsmønstre med genkendelse af gulvmarkører.
...
at implementere en kompliceret task som FollowTourTask ved hjælp af
mindre komplicerede subtasks er, at der åbnes for genbrug af disse
subtasks. Træstrukturen giver også mulighed for grafiske
...
semiautonom navigation i et miljø med visuelle
markører. FollowTourTasken er i forbindelse med eksperimenterne blevet
afprøvet med gode resultater, på en række forskellige ruter.

Det vises desuden, med eksemplerne i~\ref{sec:platformsadgang}, at
...
udvikling uden en AR.Drone tilstede.

\vspace{0.5cm}
\paragraph{Udledning af abstrakte features}ved hjælp af AR.Dronens
sensorstrømme har været hovedtemaet i de udførte eksperimenter. En
stor del af de udførte eksperimenter har omhandlet lokalisering. Under
denne process er der udviklet en visuel markør,
figur~\ref{fig:markers}, som konsistent kan genkendes med AR.Dronens
bundkamera. Markøren og FollowTourTasken, som er udviklet sideløbende,
muliggør AR.Dronens navigation efter en planlagt rute.

Efter eksperimenter med lokalisering vha. WIFI-fingerprinting blev
denne teknik fundet for upræcis. Der er teknikker til at forbedre
resultaterne, men teknikken er meget følsom og kræver et større
eksperiment-setup vi har haft mulighed for. Det forudgående arbejde
med at få tilkoblet en USB-WIFI-adapter til AR.Dronen, har dog vist,
at det generelt er muligt at udvide AR.Dronen med USB-enheder.

\vspace{0.5cm}
Eksperimenterne med korridordetektion, sektion~\ref{sec:korridor}, har
vist meget lovende muligheder, både for at kunne detektere af
AR.Dronen faktisk befinder sig i korridor og for at kunne navigere via
en sådan korridor. Korridordetektoren er oftest i stand til at
...
af faktorer der kan tages højde for.
...
AR.Dronens tilbagelagte afstand. Resultaternes ensartethed er meget
afhængig af miljøfaktorer som f.eks. belysning. En
dead-reckoningalgoritme kun baseret på denne afstand og en retning vil
ikke være pålidelig, men afstanden kunne f.eks. anvendes til at
visualisere AR.Dronens omtrentlige position på
et digitalt oversigtskort.

\vspace{0.5cm}
På trods af at resultaterne med afstandstriangulering ikke var
overbevisende, se sektion~\ref{sec:afstand}, er det ikke umuligt, at
denne teknik kan forfines og producere bedre resultater. Teoretisk er
der adgang til alle de nødvendige parametre i
trianguleringsberegningen og mulige forbedringer til teknikken
beskrives sidst i sektionen.

\vspace{0.5cm}
\paragraph{En vurderingen af AR.Dronen}gives i detaljer i
sektion~\ref{ssec:dronerecap}. Generelt vurderes AR.Dronen som et godt
alternativ til at bygge en quadrokopter-platform fra bunden. De
indbyggede sensorer har dog på nogen punkter begrænset
udviklingsmulighederne, både for semi-autonom navigation og udledning
af features. F.eks. udelukker den begrænsede videoopløsning
lokalisering ved hjælp QR-koder og retningsbestemmelse besværliggøres
på grund af sensormisvisning (præcis disse problemer er dog udbedret i
den opdaterede AR.Drone version 2.0).
Det vurderes desuden at AR.Dronens store udbredelse vil sikre et
aktivt udviklermiljø, som både kan fungere som hjælp og inspiration under
udviklingen af projekter med AR.Dronen.
...
brugsscenarie, noget turis-happening som flyv en tur på stævnsklint
