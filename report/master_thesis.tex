...
\centering
  \begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=1.0\textwidth]{grafik/usbdevices.png}
    \caption{USB-kabel og de enheder vi har haft tilslutttet AR.Dronen.}
    \label{fig:cableandusbdevices}
  \end{subfigure}  ~ %add desired spacing between images, e. g. ~, \quad, \qquad etc. (or a blank line to force the subfig onto a new line)
  \begin{subfigure}[b]{0.48\textwidth}
    \includegraphics[width=1.0\textwidth]{grafik/usbattached.png}
    \caption{AR.Dronen med usbstick tilsluttet.\\ \ }
...
Efter vores indledende manøvrer med AR.Dronens USB-port, valgte vi at
...
Vi har valgt at udstyre AR.Dronen med en Dlink DWL-G122*
wifi-adapter. Denne specifikke adapter bygger på et Ralink* chipset og
...
dog ikke er installeret på AR.Dronen som standard og skal først
kompileres til ARM-arkitekturen for at vi kan linke til det og senere
...
fra AR.Dronen anvender vi biblioteket Psyco* til at netop øge
...
inspiration fra ~\ref{derp} implementeret en virtuel sensor der bygger oven på videoreceiveren. 
...
delt liste, som anvendes både til at overføre signaler og data.
...
Switching the camera mode is done immediately once the AT command is received and the current picture is finished processing. So it's theoretically possible to switch every 15th of a second. Nevertheless this mechanism was meant to be used manually by the iPhone user once in a while; doing it by software at high frequency will result in an unpredictable number of switches.
If what you want to do is receiving pictures from both cameras at the same time by switching camera at every frame reception, there is a big chance it does not work reliably.

Stéphane Piskorski
https://projects.ardrone.org/boards/1/topics/show/560
