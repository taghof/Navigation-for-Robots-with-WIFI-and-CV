...
WIFI-sensor, som beskrevet i afsnit~\ref{ssec:wikilokalisering}. Pointen er at man ikke kan bruge
...
afsnit~\ref{sec:QRdecoding}, er billedkvaliteten ikke tilstrækkelig
...
beskrevet i header filen \uri{navdata\_common.h}, \cite{ardroneorg:navdatacommonh} fra Parrots åbne API.

\subsubsection{Videoreceiver}
Videoreceiveren modtager og afkoder AR.Dronens videostrøm. Afkodningen
foregår på baggrund af den process der nævnes i sektion 7.2 side 43 i
\cite{techreport:ardroneDevGuide}. Formatet der sendes i minder meget
om JPEG formatet og 
forfatteren til den algoritme vi har tilpasset, spekulerer
i på forumsiden, \cite{electronic:parrotforum2}, om det ville være muligt at anvende
...
end de rå værdier fra receiverstrømmene. For
...
frontkamera \todo{hvorfor to ** morten?}. Dette giver imidlertid problemer for AR.Dronens originale
...
Python-interfacet. Udover at virke i platformen kan interfacet til
...
\todo{morten? tjek semantik task istedetfor opgaver på autocontroller}
...
\subsection{Mapcreator}
\label{sec:mapcreator}
\subsection{Taskcreator}
...
Mapcreator-værktøjet beskrevet i \todo{\\ref{}}. For at teste forskellige de lokaliseringsmetoder har vi
anvendt HoverTrackTasks(se section \ref{sssec:hovertrack}) og
...
\subsubsection{WIFI-fingerprinting algoritme}
\label{sssec:wifialgo}

%http://ieeexplore.ieee.org/xpl/login.jsp?tp=&arnumber=1381942&url=http%3A%2F%2Fieeexplore.ieee.org%2Fxpls%2Fabs_all.jsp%3Farnumber%3D1381942
...
eksempel argumentation på hvorfor AR.Dronen ikke virker som videnskabelig platform, for stor og klumpet, ny forskning i quadrokopter viser det også, eg. swarms med mikrokopter.
