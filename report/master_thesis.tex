...
Styrken på radiosignalet som en WIFI kilde sender med måles i
dB. Signalstyrken 

For at teknikken skal fungere antager snifferprogrammet at det
anvendte trådløse interface er sat op i monitor og 
promiscous\footnote{Normalt vil et netværksinterface frasortere og
  ikke videresende datapakker der ikke er adresseret til processoren,
  men i promiscous mode sendes al trafik videre.} mode så det modtager alle pakker der sendes fra de
omkringliggende kilder. Netværksinterfacet indstilles gennem et
script (load.sh, \cite{github:loadsh}) som kaldes i forlængelse af AR.Dronens 
oprindelige bootup-sekvens. I dette script loades også alle de
nødvendige kernemoduler.

\subsubsection{WIFI-sensor hardware}  
Vi har valgt at udstyre AR.Dronen med en Dlink DWL-G122 WIFI-adapter,
\cite{electronic:dwlg122}. Denne specifikke adapter bygger på et
Ralink chipset, \cite{electronic:wikiralink} og
...
kompatibel med linuxkernen anvendt på AR.Dronen. Når driveren er indlæst, genkendes adapter og 
interfacet \emph{ra0} oprettes i \code{/sys/class/net/}. 
...
dog ikke installeret på AR.Dronen som standard og skulle først
kompileres til ARM-arkitekturen før vi kunne linke til det og senere
anvende programmet på AR.Dronen, se \cite{blog:compiling}.
