...
forventer på et standard linux system(\code{cd}, \code{ls}, \code{mv}, \code{ln} osv.), men
leverer desuden også en Dynamic Host Configuration Protocol (DHCP) service; den DHCP-server AR.Dronen anvender til at uddele %DHCP cite?
IP-adresser, den telnet-server der giver adgang til AR.Dronens terminal %telnet cite?
samt den editor(Vi) der gør det muligt at redigere filer på AR.Dronen. 
...
software betragtes programmet som en lukket kasse, vi kan dog udlede forskellige elementer efter at have observeret
...
\item en algoritme
til at udlede AR.Dronens hastighed ved hjælp af bundkameraet %cite hvor
...
reality spil. 
\end{itemize}
Program.elf modtager kommandoer fra brugeren gennem en User Datagram Protocol (UDP) 
...
Kommunikationen med AR.Dronen kan indeles i to kategorier, den der
beskæftiger sig med input til, og output, fra program.elf(flyvning) og
den der omhandler den almindelige anvendelse af AR.Dronens linux
system.
\subsubsection{TCP baseret kommunikation}
\label{sssec:tcp}
For at få adgang til en terminal på AR.Dronen anvendes
en telnetklient. Efter forbindelse til AR.Dronens ip-adresse får brugeren
adgang til en rodterminal, da systemet er sat op som et
enkeltbrugersystem.
\\For at overføre en fil til AR.Dronen anvendes en FTP-klient. Efter
forbindelse til AR.Dronen, kan filer overføres mellem AR.Dronen og brugerens
