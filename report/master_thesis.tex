...
En ekstern klients (en PC, smartphone eller lignende) interaktion med
...
\begin{framefig}
  ...
  \caption{AR.Dronen kommunikerer med klienter over netværket gennem UDP og TCP.}
...
den første kategori herunder og sommetider som værende i begge kategorier:
\begin{itemize}
  ...
  \item Intern interaktion med AR.Dronens interne styreprogram gennem
...
\begin{framefig}
  ...
  \caption{Udover eksterne klienter der kommunikere over det trådløse
    netværk, kan processor også kommunikere internt på AR.Dronen}
\end{framefig}
%
...
Den ene virtuelle sensor, PositionDetector,~\cite{github:virtuel},
...
istedet for behaviors, der ellers både bruges i artiklen 'A Modular Hierarchical Behavior-Based Architecture',
~\cite{lenser:behaviorbasedarchitecture}, og i Lejos' subsumption-pakke, \cite{lejossub}. Grunden til dette er, at flere af vores tasks er
forholdsvis kortlivede(for eksempel take-off og landtasks) og derfor
mere minder om en opgave der skal forberedes, udføres og
afsluttes. Dette ses i modsætning til en behavior, som kan opfattes
som en længerevarende opførsel. Vi har også implementeret højniveau bevægemønstre der
minder mere om en traditionelle behaviors (for eksempel
FollowTourTask). Vi mener også at det giver mening, at kalde disse
...
herefter stillet til rådighed for andre tasks, gennem en delt kontekstliste. I
de endelige udgaver er disse tasks dog udskilt til den virtuelle
sensor, så tasks ikke skal behandle sensordata, men kun reagere og
formidle bevægelse på baggrund af højniveau-sensordata. 

\subsection{Task typer} 
Vi har to hovedtyper, simple og compoundtasks. Simple tasks nedarver
fra basetaskklassen Task. De repræsenterer oftest en simpel
bevægelse, men kan være vilkårligt komplekse. For eksempel har vi en
task der, blandt andet, implementerer en PID-controller. 
CompoundTasks er tasks der indeholder en liste af subtasks og funktionalitet til
at starte og stoppe disse subtasks. Vi opdeler CompoundTasks
yderligere i to typer, sekventielle og parallelle. Sekventielle CompoundTasks
starter deres subtasks en efter en og venter med at starte en ny
subtask før den forrige er afsluttet. En sekventiel CompoundTask
stopper når den sidste subtask er afsluttet. Parallelle CompoundTask
starter alle deres subtasks på en gang og stopper når den
sidste subtask er afsluttet. 

\subsection{Task hieraki}
Ved hjælp af de ovenfor beskrevne tasktyper, og specialiseringer af
disse, kan vi danne en task-træstruktur med parents og children. Strukturen minder om den beskrevet i
\cite{lenser:behaviorbasedarchitecture}. Det grundliggende formål med
denne træstruktur er at danne et task-hieraki. Taskmanageren skal
ved hjælp af task-hierakiet sørge for, at der aldrig er mere end én task
...
konstrueret med vores taskcreator, beskrevet i sektion~\ref{sec:taskcreator}. Under antagelse af, at hver TestTask
ønsker at udføre et antal bevægelser med AR.Dronen og ønsker at udføre disse
bevægelser uden pause, vil TestTask med level 1 slutte
...
Et mere realistisk og komplekst eksempel end det på
figur~\ref{fig:taskhieraky}, er en FollowTourTask, hvis tasktræ kan ses
på figur~\ref{fig:followtour}. En FollowTourTask nedarver fra en sekvensiel
compoundtask og indeholder, udover takeoff og land-tasks, en
...
map-objekt, se sektion~\ref{sec:mapcreator}, og sørger for at opdatere HoverTrackTasken med en ny
...
AR.Dronen indenfor en specifik afstand af den præcise position, før
...
HoverTrackTasken implementerer en PID-controller,\cite{electronic:pid}, der
kan holde AR.Dronen over en position.
Når en markør(se \ref{fig:advancedmarker}) er synlig for AR.Dronens bundkamera, kan vi ved
hjælp af PositionDetectoren(se sektion~\ref{sec:sub:virtuel}), få
markørens position på billedet. Med denne position udregnes en
error-værdi i x og y-retningerne i forhold til centrum af billedet,
disse værdier er i pixels. For at kunne bruge errorværdierne, skal de
konverteres til millimeter og der skal tages højde for AR.Dronens
...
AR.Dronen. PID-funktionaliteten er implementeret i HoverTrackTaskens
...
\subsection{AvoidTask}



\subsection{Task implementationer}
Vi har lavet to forskellige udgaver af vores
task-system(tasks og taskmanager), som begge implementerer den
opførsel og struktur der er beskrevet ovenfor. 

Den første, trådbaserede, implementation er inspireret af Lejos' subsumption-pakke,
~\cite{lejossub}. Lejos er en Firmwareerstatning til LEGO Mindstorms
RCX- og NXT-platform, der tillader afviklingen af Java på disse
platforme,~\cite{lejos}.
Den ikke-trådbaserede implementation er primært inpireret af artiklen 'A Modular Hierarchical
Behavior-Based Architecture',\cite{lenser:behaviorbasedarchitecture}.

\paragraph{Trådbaseret} 
Den første implementation er en trådbaseret løsning, dette betyder at
hver task er implementeret som en individuel 
tråd. I den trådbaserede implementation kører alle tasks samtidig. Når
en task ønsker at bevæge AR.Dronen, får den tilladelse til dette ved
at spørge sin parent(som muligvis skal spørge videre op i
træet). 

Denne tilgang minder meget om arkitekturen der anvendes af
Lejos' subsumption pakke, \cite{lejossub}. Taskmanageren vil som tasktræets rod, have samme
rolle som Arbitratoren i Lejos system. Denne arkitektur har primært et 'bottom-up'-flow, da det er de individuelle tasks
selv der indhenter tilladelse, til at bevæge AR.Dronen, ved deres
parent. En ulempe ved designet er dog, at flowet ikke kun går mod
roden. En del kommunikation er nødvendig for sikre, at lavere-level tasks bliver
deaktiveret når en højere-level task tager kontrollen. Vores erfaring
med denne arkitektur er desuden, at den ikke skalerer særligt godt, på
grund af overheadet ved at have en tråd per task. 

\paragraph{Ikke-trådbaseret}
Tilgangen i 'A Modular Hierarchical Behavior-Based
Architecture',\cite{lenser:behaviorbasedarchitecture}, er at en
aktiveret behavior skal bestemme hvilke af sine underbehaviors der
skal aktiveres. I artiklen er dette er praktisk, da deres robot kan udføre forskellige bevægelser
på samme tid. Denne tilgang sikrer at det bedste sæt af parallelle
behaviors kan bestemmes og herefter aktiveres.
...
egnede subtask, da vi ikke opererer ikke med flere aktive tasks samtidig\footnote{De
  bevægelser vi har til rådighed kan ikke udføres uden indflydelse på 
  hinanden}. Vi definerer altid den bedst egnede subtask som den med
det højeste level og et ønske om at bevæge AR.Dronen.

Den eneste tråd i denne implementation er taskmanageren, som
kontinuerligt lader den bedst egnede task bevæge AR.Dronen. I Praksis
sker dette ved at taskmanageren vedligeholder en sorteret liste over de
aktive tasks. Under hver iteration kalder taskmanageren
domove()-metoden på den første task i listen(tasken med det højeste level), hvis metoden returner
False forsøges med den næste task i listen, hvis der returneres True
afbrydes iterationen og der startes forfra. Kaldet til
domove()-metoden propageres rekursivt ned gennem tasktræet og
returnerer True hvis AR.Dronen blev bevæget og False hvis ingen task
havde interesse i at bevæge den. Sammenlignet med
den trådede udgave, er flowet 'top-down', da bevægelsen altid
initieres af taskmanageren.    

Denne ikke-trådbaserede tilgang virker mere velegnet til en træbaseret
taskstruktur som vores. Kommunikationen mellem tasks er meget
simplere og overheadet pga. de mange tråde undgåes. 
