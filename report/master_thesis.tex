...
%\paragraph{Ekstern klientkommunikation}\\
En ekstern klientenhedss (PC, smartphone eller lignende) interaktion med
...
\end{framefig}
%
Det bemærkes, at så længe der ikke anvendes en intern klient
(userspace process på AR.Dronens Linuxsystem), vil der
altid være tale om en form for fjernstyring af
AR.Dronen. 

%\paragraph{Intern klientkommunikation}\\
Når der ikke er tale om interaktion mellem AR.Dronen og en
ekstern klient, kan interaktionen også foregå mellem 2 processor
internt på AR.Drone-platformen, hvilket altid vil grupperes under
den første kategori herunder og sommetider som værende i begge kategorier herunder:
\begin{itemize}
  \item Et program der køre internt i AR.Drone-platformens user-space,
    og som derigennem har adgang til platformens services, heriblandt:
  \item Intern interaktion med AR.Dronens styreprogram gennem
    de 3 faste UDP porte, som også anvendes af de eksterne klienter.
\end{itemize}
...
\begin{framefig}
  \centering
  \subfigure[AR.Dronens interne platforms struktur.]{\includegraphics[width=0.25\textwidth]{grafik/busybox.png}
    \label{fig:busybox}}
  ~ %add desired spacing between images, e. g. ~, \quad, \qquad etc. (or a blank line to force the subfig onto a new line)
  \subfigure[Interfacet til AR.Dronens styreprogram som userspace program.]{\includegraphics[width=0.49\textwidth]{grafik/userspace.png}
    \label{fig:userspace}}
  \label{fig:internstruktur}
  \caption{Udover eksterne klienter der kommunikerer over det trådløse
    netværk, kan processer også kommunikere internt på AR.Dronen}
\end{framefig}
