...
ekstern serialkommunikation, \cite{electronic:molex}. Porten virker bl.a. som en On-The-Go USB-port,\cite{electronic:otg}, der primært anvendes ved opdateringer.
...
På en AR.Drone sidder de 4 propeller i hver sit hjørne af en kulfiberkryds
. Når en propel roterer om sin akse flytter den
...
giver en Bernoullieffekt,\cite{nla.cat-vn856146},%\cite{electronic:wiki_bernoulli} 
som påvirker AR.Dronen med en kraft modsat tyngdekraften (thrust),
...
henholdsvis $\theta$, $\phi$ og $psi$ er den tilhørende
rotationsvinkel. På figur~\ref{fig:drone_movements} vises hvordan en generel
quadrokopter kan bevæges omkring de tre akser, det bemærkes at x og y-akserne på
denne figur er forskudt 45$\deg$ så de flugter med motorerne, hvorimod
AR.Dronens akser er afstemt med dennes frontkamera, se figur~\ref{fig:drone_akser}.   
...
bevæges om z-aksen foregår som på figur~\ref{fig:drone_movements}d,
...
guide\cite{techreport:ardroneDevGuide}}
...
($\Theta$)-vinklen overskrider grænsen. Sker dette lukkes systemet ned,
...
undersøgt mulighederne for at anvende AR.Dronens OTG-USB \cite{electronic:otg} port til
andet end at udføre opdateringer, dette har blandt indbefattet at
kompilere kernemoduler til AR.Dronens linuxkerne,
...
og skrive og læse fra denne, dette var forholdsvis
simpelt, da driverne til dette formål er meget generiske. Proceduren
for at aktivere AR.Dronens USB-port og læse fra en USB-stick kan læses
i vores blogpost "Enabling the Drone USB Port",\cite{blog:usb}. 
\begin{framefig}
  \centering
  ...
  ~ %add desired spacing between images, e. g. ~, \quad, \qquad etc. (or a blank line to force the subfig onto a new line)
  \subfigure[AR.Dronen med usbstick tilsluttet.]{\includegraphics[width=0.45\textwidth]{grafik/usbattached.png}
...
\subsubsection{WIFIsensor hardware}  
...
\subsubsection{WIFIsensor software}
Vores første eksperimenter med hensyn til at kompilere og afvikle kode
på AR.Dronen kan ses i vores blogpost "Compiling code for the
AR.Drone",\cite{blog:compiling}, dette var simple 'hello 
...
anvende programmet på AR.Dronen, \cite{blog:compiling}.

WIFIsensor-programmet har mulighed for at tage et interfacenavn som input
...
modtage og behandle pakker. WIFIsensor-programmet sender signalstyrker og
...
\section{Klient platform} % Morten,
...
\subsubsection{Eksempel på modtagelse af videostream}
...
Til dette eksperiment anvendtes bibliotekerne pyqrcode \cite{electronic:pyqrcode} og Zbar
