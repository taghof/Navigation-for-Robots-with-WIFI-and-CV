...
\section{Afstandsbedømmelse via optical flow} %Afstandsmåling gennem Optical flow} % tt
Målet med dette forsøg var at undersøge kvaliteten af en udledningen
af afstand fra AR.Dronens frontkamera, bevægelser i planet og yaw-rotation.
Til eksperimentet er der konstrueret et program, for hvilket det
overordnede princip gennemgåes herunder.

\subsection{Triangulering i 2 dimensioner}
Forsøgsopstillingen er simplificeret til kun at foregår i planet,
så det kun er bevægelser i $x$ og $y$ retningen der iberegnes,
ændringer i højden forsøges minimeret så den parameter kan ignoreres.

Det er sidestykket $|f2t\ p2|$ i trekanten på
figur~\ref{fig:opticalafstand} der er forsøgt udregnet.
\begin{framefig}
  \centering
  ...
  \caption{Opstillingen er set oppefra. AR.Dronen har flyttet sig fra
    position $p1$ til $p2$ i $x,y$-planet på tiden
    $\Delta t$. Den grønne cirkel (f2t) er et punkt på
    væggen (feature-to-track) som følges gennem frontkameraet, oprindeligt fundet ved
    OpenCV' funktion \code{goodFeaturesToTrack}. I opstillingen ønskes
  liniestykket $|f2t\ p2|$ fundet.}
  \label{fig:opticalafstand}
\end{framefig}


\subsection{Fra pixel-position til virkelig retning}
En feature følges ved at den har en fast indgang, $i$, i den liste der
returnes af funktionen til beregning af optical-flow (funktionen gennemgåes i
sektion~\ref{sec:sub:feature-tracking}). Imellem hvert nyt billede
laves en ny beregning af optical flow, og feature-indgangene i listen opdateres med nye 
$(x_i,y_i)$-pixelpositioner.

Af en features $x$-position i et billede kan man udregne vinklen i den
virkelige verden i forhold til retningen på AR.Dronens frontkamera, se
vinklen $A_{f2t i}$ i figur~\ref{fig:f2kvinkel}.
\begin{framefig}
  \centering
...
features\footnote{En egnet feature er et karakteristisk 
  punkt i billedet som kan genkendes hvis billedet flytter
  fokuspunkt. En feature kan være flere ting, bl.a. 2. ordens
  gradient over af en hvis magnitude, hjørner på objekter,
  karakteristisk teksture, osv.} (ft2) een gang i billedramme01 med OpenCV funktionen
...
\emph{calcOpticalFlowPyrLK}. Når frontkameraet har skiftet så meget
fokus at der ikke længere er overlap mellem det nuværende billede og
det oprindelige billederamme01, foretages en ny indhenting af egnede
...
\subsection{Yaw som midlertidig fælles reference}
Der er imidlertid et problem i at, AR.Dronens front nærmest aldrig peger i den
samme retning (i figur~\ref{fig:opticalafstand} ses AR.Dronens front
peger i forskellige retninger ved de to positioner $p1$ og $p2$). For
at triangulere skal man bruge 2 pejleretninger med fælles 
...
flyvning. Tanken er istedet at navdataens yaw-værdi, over korte
...
\begin{framefig}
  ...
  \caption{Den beregnede afstand ud til et fast punkt på væggen i en filmsekvens over
    ...
    $x$-aksen er millimeter, mens $y$-aksen er billedrammenummeret.}
...
\section{DistanceTracker} % Thomas
...
\begin{framefig}
  \centering
  \includegraphics[width=0.79\textwidth]{grafik/3mxdistribution.png}
  \caption{Kumulative distribution af afstande målt ved at integrere over AR.Dronens udledte hastighed i fremadrettet flyveretning.
...
\begin{framefig}
  \centering
  \includegraphics[width=0.89\textwidth]{grafik/3mydistribution.png}
