...
specifikt implementeres i første omgang en let anvendelig softwareplatform der kan
udnytte AR.Dronens indbyggede sensorapparat. Platformen anvendes
herefter til de videre eksperimenter. Det undersøges hvorledes
sensorapparatet ombord på AR.Dronen kan udnyttes, til at udlede mere
abstrakt information om miljøet i den umiddelbare nærhed. Denne
information anvendes til at lade AR.Dronen navigere i henholdsvis
korridorer og åbne rum med visuelle markører på gulvet.
...
\subsection{Python}
Vi valgt at skrive klientplatformen i Python. Grunden til dette er
primært Pythons læsbarhed og simple syntaks. Læsbar kode fordrer ikke bare
...
erfarne udviklere. Da vi har haft som mål, at skabe en let anvendelig
og let udbyggelig klientplatform, er Python det oplagte valg.
...
implementationen af vores klientværktøjer,
\cite{github:sensordisplay}\cite{github:mapcreator}\cite{github:taskcreator},
for eksempler der anvender klientplatformen.  

Udfordingen ved valget af implementationssprog er, at Python som
et dynamisk typet, fortolket sprog, ikke performance-mæssigt kan
sammenlignes med mere maskinnære sprog som C og C++. For at overkomme
denne udfordring, har vi gjort anvendelse af OpenCVs billedalgoritmer(beskrevet
nedenunder) og Psyco,~\cite{electronic:psyco}, i forbindelse med afkodning af
billeder fra AR.Dronen(se desuden sektion~\ref{subsec:receivers}). En fremtidig forbedring kunne være, at
implementere afkodningen af AR.Dronens billeder i et C-bibliotek og
bygge en Pythonwrapper til dette.    

\subsection{OpenCV}
\label{opencv}
OpenCV er et bibliotek af optimerede algoritmer, indenfor området
computer vision,~\cite{opencv}\cite{opencv:wiki}. OpenCV blev
oprindeligt startet af Intel(med bidrag fra optimeringseksperter i
Intel Russia og Intels Performance Library Team), men udvikles nu af
robotforskningslaboratoriet Willow Garage, \cite{opencv:willow:wiki}. Det har fra
starten i 1999 været et mål, at skabe ikke bare åben. men også
optimeret kode, til en grundlæggende computer vision infrastruktur.
...
Vores implementerede klientplatform kan betegnes som et system til indsamling af
AR.Dronens sensorstrømme og fjernstyring af denne. Klientplatformen
har fasciliteret de eksperimenter som beskrives i sektion~\ref{chap:Eksperimenter} og
dermed også udviklingen af de forskellige tasks i sektion~\ref{sec:tasks}. 

Klientværktøjerne Sensordisplay, Mapcreator og Taskcreator, som alle
beskrives i sektion ~\ref{chap:clienttools}, er udviklet for at lette arbejdet med
klientplatformen. Blandt andet ved at give brugeren adgang til en grafisk
repræsentation af AR.Dronens sensorstrømme. Klientværktøjerne har vist sig
meget anvendelige i forbindelse med forberedelse og udførelse af
eksperimenter med klientplatformen.

\subsection{Eksperimenter} % Thomas,
For at teste AR.Dronen og vores klientplatform, har vi udført en række
eksperimenter. Eksperimenterne omhandler generelt udledning af
kontekstuel information fra AR.Dronens sensorstrømme. Mere specifikt
tester vi metoder til lokalisering, afstandsbedømmelse, måling af
tilbagelagt afstand(Odometri) og korridordetektering.

\paragraph{Lokalisering via Wifi-fingerprinting}
Ved hjælp af en WIFI-fingerprinting algoritme, har vi undersøgt
muligheden for at lokalisere AR.Dronen i forhold til et antal
fastlagte positioner. Eksperimenternes opstilling og resultater kan
ses i sektion~\ref{ssec:wifilokalisering}.

\paragraph{Lokalisering via visuelle markører} simpel blob, og udbygget nested blob
Vi har designet et antal forskellige visuelle markører. Markørerne er
blevet testet for hvor godt de kunne genkendes af AR.Dronen, både
i statiske opstillinger og i forbindelse med bevægelse. Disse
eksperimenter har også fungeret som test af vores tasksystem
og de udviklede tasks. Eksperimenternes opstilling og resultater kan
ses i sektion~\ref{ssec:visuellokalisering}. 

\paragraph{Afstandsbedømmelse via optical flow}
Vi har undersøgt muligheden for at udlede afstande, til objekter
synlige for AR.Dronens frontkamera. 
...
% evt afhjælpe med fiskeøje, så flow af featurepoints kan trackes
% længere langs siden af kopteren når man flyver forbi...

\paragraph{Korridor detektering}
\paragraph{DistanceTracker}
...
\label{sec:blog}
Vi har i løbet af vores speciale, anvendt en blog til at beskrive
tekniske fremgangsmåder og til at fungere som adgangspunkt til vores offentlige
kode-repository. Bloggen hostes på Github og adressen angives
i~\cite{blog}. Bloggen indeholder udover en projektbeskrivelse, især informationer om vores tidlige
forsøg med AR.Dronen. Blandt andet beskrives proceduren for at
aktivere AR.Dronens USB-port i detaljer. Denne procedure har vi
stykket sammen fra adskillige kilder og vi håber med bloggen, at kunne
tilbyde en mere samlet kilde for andre i vores situation.
Et andet emne som behandles på bloggen er afvikling af kode på
AR.Dronen, herunder proceduren for cross-compiling til ARM arkitekturen. 
...
\section{Kommunikation mellem AR.Dronen og klienter} %mellem AR.Dronen
                                %og en klient-enhed
Den forbindelse der skal til for at fjernstyre AR.Dronen via en iPhone
er den oftest brugte forbindelse, men en af mange
interaktionsmuligheder. Herunder listes nogle af de muligheder
...
process der køre parallelt med AR.Dronens styreprogram på AR.Drone-platformen.
...
og et snifferprogram (\emph{WIFI-sensor}) til at aflæse og videresende signalstyrkerne fra AR.Dronens
...
forprogrammerede sekvenser af bevægelser som beskrives nærmere i
...
AR.Dronen. Herunder vises simple eksempler på hvordan klientplatformen
kan anvendes. 
...
AR.Dronen. Det antages, at computeren der afvikler eksemplet er forbundet med AR.Dronens
trådløse netværk. Fremgangsmåden ses på figur~\ref{fig:pythoninterpex1}.
...
for at modtage et billede (og endnu fire for at vise det), vi 
...
køre mellem ti og tolv minutter, såfremt vi samtidig skifter kamera
...
interfacet som en Pythontråd der løbende sender kommandoer med de
...
Man kan starte systemet med begge controllere på samme tid, eller
...
flere praktiske grunde til at vi valgte at implementere en alternativ
fjernstyring. Blandt andet havde vi et behov for at kunne interagere
både med vores eget system og AR.Dronen på samme tid, mens vi til
stadighed måtte have øjne på AR.Dronen. Vi fandt desuden ikke iPhone
styringen tilstrækkelig præcis eller særlig intuitiv. Xbox360
...
fungerer ved at oversætte værdier fra Xbox360 controlleren (som vi får
givet af pygames, \cite{electronic:pygame}, joystickmodul) og så sende
disse til kontrolinterfacet. Xbox360
Joystickcontrolleren bruges udover manuel fjernstyring af AR.Dronen
også til at starte tasks (task beskrives nærmere i sektion~\ref{sec:tasks})).  Således kan
...
istedet for behaviors, der ellers både bruges i artiklen 'A Modular
...
vores tasks er forholdsvis kortlivede (for eksempel take-off og
...
opfattes som en længerevarende opførsel. Vi har også implementeret
højniveau bevægemønstre der minder mere om en traditionelle behaviors
(for eksempel FollowTourTask). Vi mener, at det også giver mening, at
...
\subsection{Task typer} Vi har to hovedtyper, simple og
compoundtasks. Simple tasks nedarver fra basetaskklassen Task. De
repræsenterer oftest en simpel bevægelse, men kan være vilkårligt
komplekse. For eksempel har vi en task der, blandt andet,
implementerer en PID-controller.
...
Hvis man anskuer vores map-objekt som en relationel graf, hvor alle knuder er
...
erkendt position. 
Når en markør (se \ref{fig:advancedmarker}) er synlig for AR.Dronens bundkamera, kan vi ved
hjælp af PositionSilhouetDetectoren (se sektion~\ref{sec:sub:virtuel}), få
markørens position på billedet. Med denne position udregnes en
error-værdi i x og y-retningerne, i forhold til centrum af
...
med at bevæge sig op er lille (da vi kender loftshøjden), i forhold
...
\subsection{Task implementationer} Vi har lavet to forskellige udgaver
af vores task-system(taskmanageren og alle tasks), som begge implementerer
...
kontrollen. Vores erfaring med denne arkitektur er desuden, at den på
...
I vores ikke-trådbaserede tilgang forsimpler vi ovennævnte princip. Vi
siger, at en Compoundtask på opfordring skal aktivere \emph{den} bedst
...
uden indflydelse på hinanden}. Vi definerer altid den bedst egnede
subtask, som den med det højeste level og et ønske om at bevæge
AR.Dronen.
...
har vi haft brug for at kunne følge udviklingen i wifisignalstyrker
...
eksperimenter gjort brug af et map-objekt med beskrivelser af
lokationer(bestående af indsamlede WIFI-signalstyrker, markørbeskrivelse og koordinater) og en
rutebeskrivelse bestående af en liste med lokationer. Map-objektet
...
WIFI-map, som opbygges umiddelbart før eksperimentets
...
Vores setup, både det konstruerede map-objekt og den fysiske opstilling ses
...
lokationer vi finder i vores map-objekt og dermed lokalisere AR.Dronen
præcist.
...
\section{Afstandsbedømmelse vha. triangulering i planet} %via optical flow} %Afstandsmåling gennem Optical flow} % tt
For at lette læserens forståelse af forfatterens intention i dette afsnit er der forsøgt at gøre en
fælles grounding, forfatter og læser imellem, gennem nedenstående
billedelige forklaring: Eksemplet er et lignende test-scenarie, men
fra en anden verden uden flyvende quadrokoptere.
...
\begin{framefig}
  ...
  \caption{2 mennesker på en kyst kan i fælleskab bedømme afstanden
    ud til en båd på vandet, ved at kende afstanden imellem sig ($b$) og
    vinklerne imellem hinanden og båden (hhv. $a1$ og $a2$).}
  \label{fig:stranden}
\end{framefig}
Hvis de to mennesker kender den fælles afstand imellem sig, her kaldet
$b$, og vinklerne imellem hinanden og båden, kaldet $a1$ og $a2$, så
kan de i fællesskab nemt udregne afstanden ud til båden. Afstanden
imellem personen bagerst og båden i figuren vil eksempelvis kunne udregnes ved
hjælp af,~\cite{wiki:sinus}, som afstand: $A1 = \frac{\sin(a2) * b}{
  \sin(180-(a2+a1)) }$.

Ideen med det egentlige eksperiment er at få AR.Dronen til alene at agere de 2
mennesker på stranden i eksemplet herover og derigennem finde afstande
til objekter foran frontkameraet. Hvis AR.Dronen sættes til at flyve sidelæns i
et lige stykke, svarende til strandkanten herover, vil den
til 2 tider $t1$ og $t2$ være ved 2 positioner $p1$ og $p2$. Hvis
...
frontkameraet ud for de to positioner (se evt. figur \ref{fig:optflowpyrlk}).

\begin{framefig}
  \centering
  \includegraphics[width=0.85\textwidth]{grafik/strand2.png}
  \caption{AR.Drone flyver sidelæns mens frontkameraet filmer
    væggen. Den grønne cirkel ($f2t$) er en gendkendelig feature (uddybes i
    sektion~\ref{sec:sub:feature-tracking}) som optræder i hvert af de
    2 billeder taget ud for position $p1$ og $p2$. Det er afstanden mellem
    $f2t$ og $p2$ der ønskes fundet. Den grå pil i figuren viser
    AR.Dronens sidelæns flyveretning. De orange pile viser
    hvilken vej fronten på AR.Dronen peger og i hvilken retning
    frontkameraet filmer.}
  \label{fig:stranden2}
\end{framefig}
I figur~\ref{fig:stranden2} får den genkendelige feature (i den grønne cirkel) den samme
rolle som båden i figur~\ref{fig:stranden}. Kodeimplementationen er
dog ikke begrænset til at holde styr på afstanden til kun et enkelt
punkt, men tager vare på en hel liste af features i videosekvensen fra
AR.Dronens frontkamera. Fra hver feature er det muligt at udregne
afstande til AR.Dronens position. I sektionen herunder beskrives den valgte
metode til at finde, genfinde og genkende features i en videosekvens.
...
features\footnote{En egnet feature er et karakteristisk 
  ...
  karakteristisk teksture, osv.} (ft2) een gang i billedramme0 med 
...
\begin{framefig}
  ...
  \caption{Et typisk egnet good-feature-to-track er som den grønne
    ring på de 2 billedrammer: Et hjørne på en billedramme af et
    billede i billedet. Den grønne cirkel fra billedet til venstre er
...
skal afstandbedømmes, ie. oftest ligger featuren i billedet, hvortil afstanden
skal bedømmes, enten til venstre eller til højre for midten af
billedet, som det ses af de to billeder i
figur~\ref{fig:optflowpyrlk}. Hvor midten af billedet svarer til
fronten på AR.Dronen.

Konkret så følges en feature ved at den har en fast indgang, $i$, i den liste der
...
Af en features horisontale pixel-position i et billede kan man udregne vinklen i den
virkelige verden i forhold til retningen på AR.Dronens frontkamera, se
...
\begin{framefig}
  ...
  \caption{
    ...
    er kendt som at være 320 px og kameraoptikken på AR.Dronens
    frontkamera er kendt til at have et horizontal udsyn på 92 $^{\circ}$. Hvis
    den horisontale afstand i pixels mellem 2 punkter $\Delta px$ i det samme billede er kendt, kan denne
    viden tilsammen give vinklen $a_{\Delta}$ imellem de 2 punkter,
    som $a_{\Delta} = \Delta px * \frac{92^{\circ}}{320 px}$.}
...
lokale interne koordinatsystem. For at få en vinkel ud som kan bruges i
trianguleringen er det nødvendigt at flytte den over i et
globalt koordinatsystem, hvor vinkler imellem positioner i den
virkelige verden har samme referencepunkt.

\subsection{Yaw som fælles reference}
Der er imidlertid et problem i at, AR.Dronens front nærmest aldrig peger i den
samme retning (i figur~\ref{fig:absoluteevinkler} ses AR.Dronens front
peger i forskellige retninger ved de to positioner $p1$ og $p2$). For
at triangulere skal der bruges 2 pejleretninger med fælles 
...
\begin{framefig}
  ...
  \caption{Opstillingen er set oppefra. AR.Dronen har flyttet sig fra
    ...
    samme nulpunkt, ved at bruge AR.Dronens yaw-sensor.}
  \label{fig:absoluteevinkler}
\end{framefig}

\subsection{Længden: 3. og sidste kendte parameter} %Triangulering i 2
                                %dimensioner}
Der mangles at kende en sidste parameter ud af 3 for at kunne bestemme
...
måle udfra. Til gengæld kendes hastigheden AR.Dronen har fløjet med for
at komme fra $p1$ til $p2$. Hastigheden fåes fra AR.Dronens navdata
som $v_x$ og $v_y$, og er hastigheden i x-aksens og y-aksens
bevægelsesretning, ie. hhv. frem-tilbage og venstre-højre.

Forsøgsopstillingen er simplificeret til kun at foregå i planet,
så det kun er frem-, tilbage- og sideværts- bevægelser der
iberegnes. Rotationer og ændringer i højden forsøges minimeret under
flyvningen fra $p1$ til $p2$, så de parametre kan ignoreres i
udledningen af længden mellem AR.Dronens 2 positioner.
\begin{framefig}
  \centering
  ...
  $b = \Delta t * ( \sqrt{ v_x^2 + v_y^2 } )$.}
% at integrere over hastigheden i x- og y- retning i tiden $\Delta t$.
  \label{fig:linieb}
\end{framefig}

\subsection{Afstanden findes}
Det er sidestykket $|f2t\ p2|$ i trekanten på
figur~\ref{fig:opticalafstand} der er forsøgt udregnet. Eftersom der
nu er fremskaffet 3 elementer af trekanten $|p1 f2t p2|$'s vinkler og
sider, er det nu muligt at alle vinkler og sidelængder i trekanten,
heriblandt den længe ønskede $|p2 f2t|$.
\begin{framefig}
  \centering
...
sande værdi ligger imellem $4,0$ og $6,5$ meter.
Dette er et godt eksempel på et af tidspunkter hvor man bør træde et skridt
...
\paragraph{Målet} for dette eksperiment er at implementere og teste 2
...
Algortimen til at detektere korridore virker ved at lede efter et billedes
...
hough-linier. Herefter forklares hvordan linierne filtreres for
tilsidst at sige noget om et tilstedeværende eller ikke
tilstedeværende forsvindingspunkt.
...
\begin{framefig}
  \centering
  \includegraphics[width=0.39\textwidth]{grafik/vanishresult.png}
  \caption{Procent resultat af test på korridor-detektor
    virkningsgrad. I testen blev der ikke observeret false-positives.}
  \label{fig:vanishresult}
\end{framefig}
Som det ses i figur~\ref{fig:vanishresult} har detektoren i ca. 10
procent af forsøgene ikke kunnet detekte placeringen af
forsvindingspunktet korrekt. 
%
Som et mål for denne sektion blev der opsat et ønske om at detektoren skulle kunne
outputte en sandsynlighed. Outputtet skulle være en sandsynlighed for hvorvidt AR.Dronen befandt
sig i en korridor. Dette mål er ikke opfyldt.
%
Med de opstillinger der blev lavet test med, er der til gengæld ikke observeret
nogle false positives, ie. detektoren har ikke fejlagtigt 
vist at der var en korridor i de steder hvor der ikke var en korridor.
Som det ser ud nu, har den nuværende implementation en virkningsgrad
på 88 procent, dvs. i 88 tilfælde ud af 100 finder detektoren den
korrekte placering af vanishingpointet.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% % % % %

\section{Distance-tracker} % Thomas
\label{sec:distance-mesu}
%Hvad vil vi vise, afvise teste?
%Hvad kan/skal resultatet bruges til?
Dette eksperiment handler om at undersøge virkningsgraden af en metode
til at tracke eller holde styr på afstanden som AR.Droenen har fløjet
over en tur. Konceptet er kendt triptælleren i biler, og
tacho-counteren i motoren på nogle kørende robotter. Indenfor navigation er der en teknologi der hedder

forsøget forsimplet tager ikke højde for rotation i planet
...
%\cite{velocity_estimation_inside_the_drone,}
%Hvordan løste vi det, undersøgte det, hvordan udførte vi eksperimentet?
Vi kan ikke r

 ved at integrere over AR.Dronens udledte hastighed i fremadrettet flyveretning.

%Hvad var resultatet?


    Den røde graf er målinger ved 
    I testopstillingen fløj AR.Dronen 3 meter ligeud, i forskellige hastigheder og med 3 forskellige førere, ie. 2 forkellige autonome styringer og 1 menneskelig via joystick.
\begin{framefig}
  \centering
  \includegraphics[width=0.95\textwidth]{grafik/3mxdistribution.png}
  \caption{Kumulative fordeling af afstande målt i fremadrettet flyveretning over 3000 millimeter.
    Målingerne er lavet ved 3 forskellige testopsætninger.}
  \label{fig:xdistr}
\end{framefig}
...
\begin{framefig}
  ...
  \caption{Kumulative distribution af afstande målt ved at integrere over AR.Dronens udledte hastighed i sideværts flyveretning.
    I testopstillingen fløj AR.Dronen kun fremad og ikke til siderne, med forskellige hastigheder og med 3 forskellige førere, ie. 2 forkellige autonome styringer og 1 menneskelig via joystick.}
  \label{fig:ydistr}
\end{framefig}
