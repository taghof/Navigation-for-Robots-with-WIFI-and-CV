...
milli-ampere-timer ($mAh$) og en afladnings hastighed på $10$ Coulomb
($C$), \cite{electronic:ar.drone_parrot_technologies}. Opladning af batteriet kan foregå i løbet af 90 minutter. Efter
sigende skulle batteriet give omking 12 minutters
flyvetid, \cite{electronic:wiki_ar.drone}.
...
\begin{framefig}
  ...
  \caption{(a) vinkelhastigheden $\Omega$ øges med $\Delta_A$ på alle
    4 propeller, quadrokopteren øger dermed løftekraften (acceleration
    i Z-aksen), (b) Vinkelhastigheden øges på venstre og mindskes på
    højre propel, quadrokopteren laver et ``roll'' til højre
    (acceleration i Euler-vinklen theta), (c) samme som b, men i pitch
    (acceleration i euler-vinkel phi) (d) En større vinkelhastighed på
    de med-uret-drejende propeller gør at torque bliver ulige og at
    quadrokopteren drejer mod venstre (acceleration i
    yaw/psi-vinklen). Ved bevægelser frem og tilbage udledes en
    positiv eller negativ hastighed ($V_x$), ligesom ved bevægelser
...
\subsection{AR.Dronens styreprogram}
...
\section{Kommunikation mellem AR.Dronen og en klient} %mellem AR.Dronen og en klient-enhed
...
\subsection{TCP baseret kommunikation}
\label{sssec:tcp}
...
ad og MAC\fotnote{Media Access Control}-adressen på pakkens afsender samt signalstyrken puttes i en ny
...
\subsection{Omkring hastighed og inerti}
...
højdemålingen.
%
\subsection{Realtidssystem} På forummet AR Drone
...
%
\subsection{Trådløs båndbredde} Der er ikke et stort overhead i User
...
prædefinerede opgaver som beskrives nærmere i sektion~\ref{sec:tasks}.

\section{Platformsadgang via Python}
Python sproget blev i første omgang valgt for lette tilgængelighed for
...
\subsection{JIT-kompileret afkodning af billeder}
%%% % % %
Da Python er et fortolket højniveau sprog kan vi
...
fra AR.Dronen anvender vi biblioteket Psyco~\cite{electronic:psyco}
til netop at øge afviklingshastigheden. Psyco er en form for JIT compiler der lader os
afvikle umodificeret Pythonkode hurtigere. Til sammenligning vil
...
Psyco valgt ikke at lave en 64-bit version af biblioteket. Et godt
future-work projekt ville være at lave en implementation af
billedeafkodningen i C eller lignende, for så at lave en
Python-wrapper til brugeren af platformen til både 32 og 64 bit
maskiner. Det vil være spændende at se om ikke det vil give en endnu
hurtigere afviklingstid.
%%%
...
gennemgåes i sektion~\ref{sec:sub:controlinterface}).
 ...
 bruges til at fortolke basedataen fra de almindelige receivere.
...
Navdatareceiveren modtager navdatastrøm(højde, hastighed m.m.) via UDP fra
AR.Dronen og decoder denne, i forhold til de strukturer der findes
beskrevet i \todo{**filnavn**} fra Parrots SDK \todo{reference cite}
navdata\_common.h
%https://projects.ardrone.org/embedded/ardrone-api/d9/d3d/navdata__common_8h-source.html

\subsubsection{Videoreceiver}
Videoreceiveren modtager og afkoder AR.Dronens videostrøm. Afkodningen
foregår på baggrund af den process der nævnes i \todo{ **AR.Drone dev
guide** sektion 7.2 side 43}. Formatet der sendes i minder meget om JPEG formatet og
...
kun giver effektivitet på 32-bit platformen.

\subsubsection{WIFI-receiver}
Som de andre receivere modtager WIFI-receiveren en datastrøm fra
...
\subsection{Kontrolinterface, AT-kommando Python interface}
...
\todo{Uddybes en del, med referencer til eksempler med AT-kommando
  ...
  fjernstyring i firkant figur~\ref{fig:pythonimpl1}}
...
Vores interface(\todo{REF TIL EGEN KODE controllers.py}) definerer en række metoder til at
...
controllere til at tage mod input fra brugeren. Via
...
controllerklasse: Controller (se figur~\ref{fig:controlleruml}). Denne er implementeret som en tråd der med et givent
...
\begin{framefig}[H]
  ...
  \label{fig:markers}
  \caption{Markører vi har testet i forbindelse med lokalisering af AR.Dronen}
\end{framefig}
...
og 2.0 meters højde og resultaterne kan ses på figur
...
en erkendt lokation når bundkameraets framerate faldt. Forsøget viste
...
har en memoryleak, dette viser sig ved at softwarens hukommelses forbrug
stiger kraftigt så snart vi iværksætter hurtige kameraskift ved brug
af AR-kommandoen set\_config.*double info beskrives også i virtuel* 
...
Eksperimentet bekræftede at AR.Dronen konstistent kunne lokalisere sig
selv. 
\begin{framefig}[H]
  \centering
...
bruge data fra frontkameraet, dette gør at vi er nødt til kontinuerligt at zappe mellem
de to kameraer og dette fungererer som nævnt i \ref{p:advanced3} ikke særligt
godt. Vi har derfor undersøgt mulighederne for at anvende detk
amerafeed hvor bundkameraets billeder er lagt oven på
frontkameraets(se \ref{fig:2frames}), når vi udvinder bundkameraets billeder fra
denne kilde får vi billeder i en opløsning på 88x72.
Eksperimenterne er fuldstændig magen til dem vi udførte i \ref{p:simple1} og
\ref{p:advanced1}. De er ligeledes udført i højderne 0.5, 1.0, 1.5 og 2.0 og er
udført både med de simple, semiavancerede og avancerede
markører. Resultaterne ses på figur \ref{fig:detection2}, de viser tydeligt at
...
frontkamera(320x240) og scanner disse for QR-koder ved hjælp af Zbar.
...
at vi derfor må anvende andre algoritmer til lokalisering i vores system.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% % % % %
%intro
Eksperimenterne herover har ind til videre omhandlet tilgange til det
at genkende abstraktioner på konkrete absolute lokationer. Denne
sektion beskæftiger sig istedet med udledningen af til relativ viden\todo{andet ord}
fra rå sensordata.

i form af udledning af højere abstrakt viden fra rå data, eller viden
af lavere abstraktionsniveau ifølge Dey01 et al., \cite{Dey01:ctoolkit}.
context-widget). 
Et eksempel på
relativ viden er hastigheden $v_x$ og $v_y$ som er tilgængelige gennem interfacet til
AR.Dronen. Hastigheden er udledt af 
...
For senere at implementere det som en virtuel sensor som i sektion~\ref{sec:sub:virtuel}.
...
\begin{figure}
  \centering
  \includegraphics[width=0.99\textwidth]{grafik/vanish_operation1.png}
  \caption{(a) Et billede af en gang påført gauss-sløring (b) Billede a efter kantdetektering (her et sobel-filter) (c) Hough-linier udregnes udfra kantpunkterne i billede b (d) Vandrette og lodrette hough-linier sorteres fra billede c.}
...
hacker, tinker, DIY fora.  Tilgængel nogle videnskabelige artikler
