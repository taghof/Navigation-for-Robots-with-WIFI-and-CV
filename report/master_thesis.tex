...
nemt udregne afstanden ud til båden. Afstanden imellem personen
bagerst og båden i figuren vil eksempelvis kunne udregnes ved 
hjælp af,~\cite{wiki:sinus}, som afstand: $A1 = \frac{\sin(a2) * b}{
  \sin(180-(a2+a1)) }$.

Ideen med det egentlige eksperiment er så at få AR.Dronen til alene at agere de 2
mennesker i eksemplet herover og derigennem finde afstande
til objekter foran frontkameraet. 

Hvis AR.Dronen sættes til at flyve sidelæns i et lige stykke, svarende til strandkanten herover, vil den
...
\begin{framefig}
  ...
  \caption{AR.Drone flyver sidelæns mens frontkameraet filmer
    ...
    2 billeder. Billederne er taget ud for position $p1$ og $p2$. Det er afstanden mellem
...
rolle som båden i figur~\ref{fig:stranden}, mens AR.Drones 2
...
features\footnote{En egnet feature er et karakteristisk 
  ...
  karakteristisk teksture, osv.} $(ft2_i)$ een gang i billedramme0 med 
...
Af en features horisontale pixel-position i billedet kan man udregne vinklen i den
...
positioner har samme referencepunkt og kan bruges i den virkelige verden.
...
med imellem $p1$ til $p2$. Hastigheden fåes fra AR.Dronens navdata 
som $v_x$ og $v_y$, og er hastigheden i x-aksens og y-aksens
bevægelsesretning ( hhv. $v_{\updownarrow}$ og $v_{\leftrightarrow}$ i
figur~\ref{fig:linieb}).
%( hhv. $v_{frem-tilbage}$ og $v_{venstre-højre}$ i figur~\ref{fig:linieb}).

Testopstillingen er forsøgt simplificeret til kun at foregå i planet,
...
\begin{framefig}
  ...
  $b = \Delta t * ( \sqrt{ v_{\updownarrow}^2 + v_{\leftrightarrow}^2 } )$.}
...
Det er hensigtsmæssigt at have kontekstuel viden for at kunne
...
blive en del af et virtuelle sensor hieraki.
...
\begin{framefig}
  \centering
  \includegraphics[width=0.59\textwidth]{grafik/korridoreksempel.png}
...
skæringspunktet af de diagonale linier som går gennem billedet.
\begin{framefig}
  \centering
...
delskridt (se figur~\ref{fig:korridorvandfald}). Inputbilledet roteres først ift. AR.Dronens 
roll-vinkel, $\phi$ (se figur~\ref{fig:korridorphi}), herefter
beregnes linierne i billedet ved hjælp af et Canny-filter,~\cite{Canny1986}, og
Houghtransformation,~\cite{duda1972houghline} (gennemgåes i sektion~\ref{sec:introCannyHough}). 
\begin{framefig}
  \centering
  \includegraphics[width=0.99\textwidth]{grafik/korridorphi.png}
  \caption{For senere at lette frasorteringen af de vandrette og
    lodrette linier fra i den reele korridor, augmenteres billedet med
    AR.Dronens roll-vinkel $phi$. Herefter kan linier filtrerers
    via deres hældning alene.}
  \label{fig:korridorphi}
\end{framefig}
De ikke-diagonale
linier frasorteres, hvorefter skæringspunker mellem hver af de
diagonale linier beregnes. Antallet af linieskæringer tælles op
for hver celle i billedet (der er $11 \times 11$ celler per
billede, se figur~\ref{fig:phiaugmentet}).
Afhængig af om der er en korridor eller ikke, er det som sagt oftest ved cellen
med størst antal linieskæringer at forsvindingspunktet kan findes.
%
...
Canny-filter,~\cite{Canny1986}, brugt til at finde 
...
med% polar-koordinater %polar-form
. Der er lavet en
mere detaljeret beskrivelse af metoderne til at detektere kanter og
linier i appendiks~\ref{app:linier}.

Figurene~\ref{fig:vanishopa} til \ref{fig:vanishopd} illustrerer forskellige trin i
...
\begin{framefig}
  ...
  \subfigure[Vandrette og lodrette hough-linier sorteres fra billede c]{
    ...
    \label{fig:vanishopd}}
  %~ 
  \label{fig:houghprocessen}
  \caption{Detektering af Houghtransformerede linier.}
\end{framefig}
...
\begin{framefig}
  ...
  \caption{Visualisering af forsvindingspunkts-detektor.}
\end{framefig}
...
tilhørende forsvindingspunkt (at undgå false-positives), samt at der bør
være linieskæringspunkter jævnt fordelt over det meste af billedet. 
...
%\subsection{Resultater}
I 86 af de test der er lavet hvor korridorenden ses i midten af et billede, hvor billedet
er taget fra midten og ned ad Ada 100-gangen, finder detektoren
området for forsvindingspunktet hver gang. Udover de 86 er der 10
tilfælde hvor detekteringen ikke virker. Et eksempel er hvis billedet
er taget i den ene side af gangen, hvor 
Canny-filteret har detekteret mange næsten lodrette linier, men ikke lodret nok til at blive frasorteret
...
\begin{table}
  ...
  \begin{tabular}{ l | p{3,5cm} | p{5,5cm} }
    ~  & tror der er & tror ej der er \\
    \hline
    der er faktisk & 79 \% & 11 \% \\
    \hline
    der er faktisk ikke & 0 \% & 10 \% \\
  \end{tabular}
  \end{center}
  \caption{Resulter af detektering udført på 111 billeder.} 
  \label{tab:vanishresults}
\end{table}

\begin{framefig}
  \centering
  \includegraphics[width=0.8\textwidth]{grafik/out_put_6jpg_error.png}
  \caption{Eksempel på synsvinkel hvor detektoren ikke fungere
...
\end{framefig}
Som det ses i tabel~\ref{tab:vanishresults} har detektoren i ca. 10
...
rigtigt 98 gange ud af 111, hvilket giver en succesrate på 89 procent
(se tabel~\ref{tab:vanishresults}).
